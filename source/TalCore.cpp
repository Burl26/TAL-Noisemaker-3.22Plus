...
void TalCore::setParameter (int index, float newValue)
{
    ...
    // Burl26 April 2019 - for midi learn
...
inline void TalCore::processMidiPerSample(MidiBuffer::Iterator *midiIterator, const int samplePos)
{
    ...
    {
    	//Burl26 - Apr 2019 - shifted priority to note on/note off
        ...
        {
        	//Burl29 Apr 2019 Return of MIDI Controller!
        ...
        {
        	//Burl26 - April 2019 Added program change
        	int i = midiMessage->getProgramChangeNumber();
        	setCurrentProgram(i);
        }
    }
}
...
//Burl26 Apr 2019 - Return of MIDI Controller!
void TalCore::handleController (const int controllerNumber,
                                const int controllerValue)
{
    if (midiGlobalLearnFlag)
    {
		// global learn: map the last moved controller to all presets
		for (int i = 0; i < this->numPrograms; i++)
		{
			talPresets[i]->midiMap[controllerNumber] = lastMovedController;
		}
		midiGlobalLearnFlag = 0; // signal GUI to clear the MIDI learn button
    } else if (midiProgLearnFlag)
	{
		// program learn
		talPresets[curProgram]->midiMap[controllerNumber] = lastMovedController;
		midiProgLearnFlag = 0; // signal GUI to clear the MIDI learn button
	}
    // apply change to controller
...
void TalCore::restoreMidiMapping(XmlElement* e, int i)
{
	// first: clear the controller of any old mappings
	for (int j=0; j < MAX_CC; j++)
	{
		talPresets[i]->midiMap[j] = 0;
	}
    XmlElement* midiMap = e->getChildByName(("midimap"));
    if (midiMap != 0 && midiMap->hasTagName(("midimap")))
    {
        forEachXmlChildElement (*midiMap, ec)
        {
			int controller = ec->getIntAttribute(("controllernumber"), 0);
			int p = ec->getIntAttribute(("param"), 0);
			if ((controller < MAX_CC) && (controller > 0) && (p > 0) && (p < 128))
			{
				talPresets[i]->midiMap[controller] = ec->getIntAttribute(("param"), 0);
			}
        }
    }
}
void TalCore::storeMidiMapping(XmlElement* activeProgram, int i)
{
	int hasMap = 0;
    XmlElement *midiMapList = new XmlElement ("midimap");
    for (int j = 0; j < MAX_CC; j++)
    {
		if (talPresets[i]->midiMap[j] != 0)
		{
			XmlElement* map = new XmlElement ("map");
			map->setAttribute (("param"), talPresets[i]->midiMap[j]);
			map->setAttribute (("controllernumber"), j);
	...
	if (hasMap)
	{
		activeProgram->addChildElement(midiMapList);
	} else
	{
		delete midiMapList;
	}
...
// End Burl26 April 2019

void TalCore::getStateInformation (MemoryBlock& destData)
{
    // header
	curProgram = this->getCurrentProgram();
    XmlElement tal("tal");
    ...
    for (int i = 0; i < this->numPrograms; i++)
    {
    	// create the XML for the program
    ...
    //#ifdef BURL26_DEBUG
    //File *file = new File("/media/burl26/data/src/TAL3.22/Debug/presetsSave.txt");
...
//Burl26 - April 2019 - get all the programs instead of just the current one
XmlElement* TalCore::getAllProgramStateInformationAsXml()
{
    MemoryBlock destData;
    this->getStateInformation(destData);
    return this->getXmlFromBinary(destData.getData(), destData.getSize());
}
//This may be required by some hosts?
...
void TalCore::setStateInformationFromXml(XmlElement* xmlState)
{
    if (xmlState != 0 && xmlState->hasTagName("tal"))
    {
        ...
        //Burl26 - April 2019 ability to start loading program list other than at 0
        int programNumber = (int)xmlState->getIntAttribute("startprogram", 0);
        if (programNumber < 0) programNumber = 0;
        if (programNumber > 255) programNumber = 255;

        XmlElement* programs = xmlState->getFirstChildElement();
        if (programs->hasTagName("programs"))
        {
            // if there are lots of programs
            if (programs->getNumChildElements() != 1)
            {
				...
				forEachXmlChildElement (*programs, e)
				{
					...
					programNumber++;
					if (programNumber == 256) break;
				}
            }
            else
            {
            	// if there is only one program, load it into the current program
...
void TalCore::getXmlPrograms(XmlElement *programList, int programNumber)
{
        ...
        //Burl26 Aprl 2019 - add the XML for the MIDI controller
        storeMidiMapping(program, programNumber);
...
void TalCore::setXmlPrograms(XmlElement* e, int programNumber, float version)
{
    if (e->hasTagName("program") && programNumber < this->numPrograms)
    {
        ...
        //Burl26 -April2019 - restore MIDI mapping
        restoreMidiMapping(e, programNumber);
...
{
    // header
	//Burl26 April 2019  Why doesn't this work?
	curProgram = this->getCurrentProgram();
    XmlElement tal("tal");
    tal.setAttribute ("curprogram", curProgram);
    tal.setAttribute ("version", 1.7);
...
void TalCore::renameProgram(String s)
{
	talPresets[curProgram]->name = s;
...
//Burl26 April 2019 - New Function
void TalCore::initCurrentProgram()
{
    talPresets[curProgram]->Initialize();
