...
//Burl26 - maximum number of CCs in MIDI is 128 so why go higher?
#define MAX_CC 128
...
class TalPreset
{
  public:
    ...
    //Burl26 - Moved initiator code into callable function
    void Initialize()
    {
        // Init default values
		// Set all values to 0 except those below
		for (int i = 0; i < TAB1OPEN; i++)
        {
			programData[i] = 0.0f;
		}
        ...
        programData[OSC2FINETUNE] = 0.5f;
         //    master
        programData[OSC1VOLUME] = 0.5f;
        programData[OSC2VOLUME] = 0.5f;
